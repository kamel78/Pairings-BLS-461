...
fn bench_pairing_miller_loop(b: &mut ::test::Bencher) {
    ...
    let mut count = 0;
    b.iter(|| {
        let tmp = CurveEngine::miller_loop(&[(&v[count].0, &v[count].1)]);
        count = (count + 1) % SAMPLES;
        tmp
    });
}
...
fn bench_pairing_final_exponentiation(b: &mut ::test::Bencher) {
    ...
    let v: Vec<Fq12> = (0..SAMPLES)
        ...
        .map(|(ref p, ref q)| CurveEngine::miller_loop(&[(p, q)]))
        .collect();

    let mut count = 0;
    b.iter(|| {
        let tmp = CurveEngine::final_exponentiation(&v[count]);
        count = (count + 1) % SAMPLES;
        tmp
    });
}
...
fn bench_pairing_full(b: &mut ::test::Bencher) {
    ...
    let mut count = 0;
    b.iter(|| {
        let tmp = CurveEngine::pairing(v[count].0, v[count].1);
        count = (count + 1) % SAMPLES;
        tmp
    });
}
